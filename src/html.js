  ...
  <div class="page" id="page-rules">
    ...
    <div style="display:grid;grid-template-columns:minmax(0,1.4fr) minmax(0,1fr);gap:16px;align-items:start" id="rulesTwoCol">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:12px">
        <h3 style="margin:0">📚 规则库 <span style="color:var(--text2);font-size:14px;font-weight:normal">从 MetaCubeX 分类中挑选加入分流规则</span></h3>
...
async function checkSession() {
  try {
    // 首屏 bootstrap：登录态 + 用户名 + 当前页数据一次拿回
    // （原来是串行 3 次往返：/auth/session → /auth/username → /dashboard）
    const data = await api('/auth/session?page=' + encodeURIComponent(state.currentPage));
    if (data.data?.authenticated) {
      ...
      if (badge && data.data.username) badge.textContent = data.data.username;
      // 首屏数据随登录态一起返回，存起来交给 switchPage→loadDashboard 直接渲染
      if (data.data.dashboard) state.dashboardPrefetch = data.data.dashboard;
      return true;
    }
  } catch {}
...
function renderDashboard(d) {
  ...
  // 最近更新时间
  document.getElementById('statUpdate').textContent = d.lastUpdate ? new Date(d.lastUpdate).toLocaleString() : '暂无';
}

async function loadDashboard() {
  try {
    // 首屏数据可能已随 bootstrap 一起返回，命中后不再重复请求
    const prefetched = state.dashboardPrefetch;
    state.dashboardPrefetch = null;
  ...
  document.getElementById('outputLinks').innerHTML = groups.flatMap(g => {
    return [
      ...
      ...g.formats.map(f => {
        ...
        const dnsNote = f.key === 'mihomo'
          ? \`<div style="margin:6px 0 12px;padding:9px 12px;color:var(--red);font-size:13px;line-height:1.6;background:var(--bg);border-left:3px solid var(--red);border-radius:6px">⚠️ 防 DNS 泄露：用 <b>OpenClash</b> 导入本配置时，请到 插件面板 → 全局设置 → DNS 设置，<b>关闭「自定义上游 DNS 服务器」</b>——否则会覆盖本订阅的 DNS 分流，导致国内 DNS 地址明文外发（DNS Leak Test 报中国 IP）。</div>\`
          : '';
...
// ============ 规则库（设置页） ============
let catalogAll = [];      // 全量分类清单（实际条数以「共 N 个分类」为准，会随上游同步增长）

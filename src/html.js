...
.rules-items { display: none; padding: 6px 20px 16px 48px; gap: 8px 12px; }
.rules-group-head.open + .rules-items { display: grid; grid-template-columns: repeat(auto-fill, minmax(210px, 1fr)); }
.rules-item {
...
.rules-item input[type="checkbox"] { accent-color: var(--accent); cursor: pointer; margin-top: 2px; flex-shrink: 0; }
.rules-item .rule-label { flex: 1; min-width: 0; overflow-wrap: break-word; }
.rules-item .rule-id { display: block; font-size: 11px; color: var(--text2); margin-top: 1px; letter-spacing: 0; }
  ...
  container.innerHTML = RULE_GROUPS.map((g, gi) => {
    ...
    const itemsHTML = g.items.map(it => {
      ...
      return \`<label class="rules-item">
        ...
        <span class="rule-label">\${it.label}<span class="rule-id">\${it.tag}:\${it.id}</span></span>
...
async function searchCatalog() {
  ...
  try {
    ...
    const url = q ? \`/rules/catalog?q=\${encodeURIComponent(q)}&limit=\${limit}\` : \`/rules/catalog?limit=\${limit}\`;

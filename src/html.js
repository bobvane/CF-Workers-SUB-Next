...
<script>
  ...
  function renderOutputUrls(total) {
    ...
    listEl.innerHTML = formats.map(f => {
      const url = token ? \`\${baseUrl}?format=\${f.key}&token=\${token}\` : \`\${baseUrl}?format=\${f.key}\`;
      return \`<div class="sub-item">
  ...
  async function copyAllUrls() {
    ...
    const urls = formats.map(f => token ? \`\${baseUrl}?format=\${f}&token=\${token}\` : \`\${baseUrl}?format=\${f}\`);
  ...
  async function copyAllUrls() {
    ...
    const token = document.cookie.match(/token=([^;]+)/)?.[1] || '';
    const formats = ['auto', 'clash', 'base64', 'singbox', 'surge', 'loon'];
    const urls = formats.map(f => \`\${baseUrl}/sub?format=\${f}\${token ? '&token=' + token : ''}\`);
  ...
  async function previewConfig() {
    ...
    try {
      const token = document.cookie.match(/token=([^;]+)/)?.[1] || '';
      const url = '/sub?format=' + format + (token ? '&token=' + token : '');
      const res = await fetch(url);
      const text = await res.text();
      pre.textContent = text;
    } catch (e) {

...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .rule-group-title { padding: 12px 20px 8px; font-size: 12px; font-weight: 600; color: var(--text2); text-transform: uppercase; letter-spacing: .5px; cursor: pointer; user-select: none; }
  .rule-group-title:hover { color: var(--text); }
  .rule-group-title::before { content: '▼ '; }
  .rule-group-title.collapsed::before { content: '▶ '; }
  .rule-group-title.collapsed ~ .rule-item { display: none; }
  .rule-item {
  ...
  .rule-item .toggle input:checked + .slider { background: var(--green); }
  .rule-item .toggle input:checked + .slider::before { background: #fff; transform: translateX(16px); }
  .rule-item .toggle input:disabled + .slider { background: var(--red); opacity: 0.8; cursor: not-allowed; }
  .rule-item .toggle input:disabled + .slider::before { background: #fff; transform: translateX(16px); }
  .rule-item .info { flex: 1; min-width: 0; }
  .rule-item .info .name { font-size: 14px; font-weight: 500; display: flex; align-items: center; gap: 6px; }
  ...
  .rule-item .actions { display: flex; gap: 4px; margin-left: 4px; flex-shrink: 0; }
  .rule-item .actions button { background: none; border: 1px solid var(--border); color: var(--text2); width: 26px; height: 26px; border-radius: 4px; cursor: pointer; font-size: 12px; display: flex; align-items: center; justify-content: center; }
  .rule-item .actions button:hover { border-color: var(--accent); color: var(--text); }
  .rule-item .actions .del:hover { border-color: var(--red); color: var(--red); }
  .locked-badge { font-size: 10px; padding: 1px 5px; border-radius: 3px; background: color-mix(in srgb, var(--red) 20%, transparent); color: var(--red); font-weight: 400; margin-left: 6px; }
  .extra-rules { display: none; }
  .scroll-btns { position: fixed; bottom: 20px; right: 20px; display: flex; flex-direction: column; gap: 4px; z-index: 50; }
  .scroll-btns button { width: 36px; height: 36px; border-radius: 50%; border: 1px solid var(--border); background: var(--bg2); color: var(--text2); cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px var(--shadow); }
  .scroll-btns button:hover { background: var(--bg3); color: var(--text); }
  ...
  .section h3 { font-size: 13px; font-weight: 600; color: var(--text2); margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.5px; }
  ...
  .modal-content { background: var(--bg2); border: 1px solid var(--border); border-radius: 12px; width: 90%; max-width: 800px; max-height: 90vh; overflow: hidden; display: flex; flex-direction: column; }
  ...
  .modal-header .close { cursor: pointer; color: var(--text2); font-size: 18px; }
  .modal-header .close:hover { color: var(--red); }
  .modal-body { padding: 16px; overflow: auto; }
  .modal-body select { padding: 8px 12px; border-radius: 6px; border: 1px solid var(--border); background: var(--input-bg); color: var(--text); margin-bottom: 12px; }
  .modal-body pre { background: var(--bg); border: 1px solid var(--border); border-radius: 6px; padding: 12px; font-size: 12px; line-height: 1.5; white-space: pre-wrap; word-break: break-all; color: var(--text); }
...
<div class="login-page" id="loginPage">
  <div class="login-card">
    ...
    <label>管理密码</label>
    <input type="password" id="loginPwd" placeholder="输入管理密码" onkeydown="if(event.key==='Enter')login()">
...
<div class="app" id="app">
  <aside class="sidebar">
    ...
    <nav>
      <a class="active" data-page="subscribe">🔗 订阅管理</a>
      <a data-page="output">⚙️ 输出配置</a>
      <a data-page="rules">📋 规则管理</a>
  ...
  <main class="main">
    ...
    <div class="page" id="pageOutput" style="display:none">
      <div class="card">
        <div class="card-header"><h2>📤 输出配置</h2><span class="badge" id="outputNodeCount">0 个节点</span></div>
        
        <div class="section">
          <h3>🔗 订阅地址</h3>
          <p style="font-size:13px;color:var(--text2);margin-bottom:12px;">
            复制下方链接到 OpenClash / Clash / Sing-box / Surge / Loon 等客户端
          </p>
          <div class="sub-list" id="outputUrlList">
            <div style="padding:16px;text-align:center;color:var(--text3);">加载中…</div>
          </div>
        </div>
        ...
        <div class="section" style="margin-top:20px;">
          ...
          <div class="output-actions">
            <button class="primary" onclick="previewConfig()">👁 预览配置</button>
            <button class="secondary" onclick="refreshNodeHealth()">🔄 刷新节点数</button>
          </div>
        </div>

        <div class="modal" id="qrcodeModal">
          <div class="modal-content" style="max-width:360px;">
            <div class="modal-header"><h3>📱 扫码订阅</h3><span class="close" onclick="closeModal('qrcodeModal')">✕</span></div>
            <div class="modal-body" style="text-align:center;padding:24px;">
              <div id="qrcodeImage" style="margin:0 auto 16px;width:240px;height:240px;background:var(--bg);border-radius:8px;display:flex;align-items:center;justify-content:center;color:var(--text3);font-size:13px;">加载中…</div>
              <p id="qrcodeUrl" style="font-size:12px;color:var(--text2);word-break:break-all;"></p>
              <button class="primary" onclick="copyQrCodeUrl()" style="margin-top:12px;padding:8px 20px;">📋 复制链接</button>
            </div>
          </div>
        </div>
        ...
        <div class="modal" id="previewModal">
          <div class="modal-content">
            <div class="modal-header"><h3>👁 配置预览</h3><span class="close" onclick="closeModal('previewModal')">✕</span></div>
            <div class="modal-body">
              <select id="previewFormat" onchange="previewConfig()">
                <option value="auto">自适应 (Clash YAML)</option>
              ...
              <pre id="previewContent" style="max-height:400px;overflow:auto;"></pre>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="page" id="pageRules">
      <div class="card">
        ...
        <div class="card-body" id="ruleContainer">
          ...
          <div id="commonRules">
            ...
            <div class="rule-group">
              ...
              <div class="rule-item"><label class="toggle"><input type="checkbox" checked><span class="slider"></span></label><div class="info"><div class="name">Steam</div><div class="desc">Steam 商店、社区、下载加速</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Steam.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox" checked><span class="slider"></span></label><div class="info"><div class="name">Steam CN</div><div class="desc">Steam 国区服务（直连）</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/SteamCN.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Epic</div><div class="desc">Epic 游戏商店</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Epic.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">游戏下载</div><div class="desc">游戏平台下载流量分流</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/GameDownload.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
            </div>
            <!-- ⚙️ 系统服务 -->
            <div class="rule-group">
              <div class="rule-group-title">⚙️ 系统服务</div>
              <div class="rule-item"><label class="toggle"><input type="checkbox" checked><span class="slider"></span></label><div class="info"><div class="name">Apple 服务</div><div class="desc">iCloud、App Store、Apple 全系服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Apple.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
          ...
          <div class="extra-rules" id="extraRules">
            ...
            <div class="rule-group">
              ...
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Instagram</div><div class="desc">Instagram 图片社交</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Instagram.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Discord</div><div class="desc">Discord 语音聊天平台</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Discord.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Reddit</div><div class="desc">Reddit 社区</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Reddit.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">WhatsApp</div><div class="desc">WhatsApp 即时通讯</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Whatsapp.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">KakaoTalk</div><div class="desc">KakaoTalk 韩国即时通讯</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/KakaoTalk.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Zoom</div><div class="desc">Zoom 视频会议</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Zoom.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
            </div>
            <!-- 🎬 更多流媒体 -->
            <div class="rule-group">
              <div class="rule-group-title">🎬 更多流媒体</div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">HBO</div><div class="desc">HBO Max 流媒体</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/HBO.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">HBO GO HKG</div><div class="desc">HBO GO 香港地区</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/HBO_GO_HKG.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Hulu Japan</div><div class="desc">Hulu 日本流媒体</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/HuluJapan.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              ...
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Pandora</div><div class="desc">Pandora 音乐电台</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Pandora.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Bahamut</div><div class="desc">巴哈姆特动画疯</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Bahamut.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Line TV</div><div class="desc">Line TV 台湾流媒体</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/LineTV.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">TaiWan Good</div><div class="desc">TaiWan Good 流媒体</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/TaiWanGood.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">ViuTV</div><div class="desc">ViuTV 香港流媒体</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/ViuTV.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
            </div>
            <!-- 🎮 更多游戏 -->
            <div class="rule-group">
              <div class="rule-group-title">🎮 更多游戏</div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Blizzard</div><div class="desc">暴雪游戏（守望、魔兽等）</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Blizzard.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Nintendo</div><div class="desc">任天堂 Switch 在线服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Nintendo.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Origin</div><div class="desc">EA Origin 游戏平台</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Origin.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Xbox</div><div class="desc">Xbox 游戏服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Xbox.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">PlayStation</div><div class="desc">Sony PlayStation 网络</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Sony.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
            </div>
            <!-- 🐱 国内互联网 -->
            <div class="rule-group">
              <div class="rule-group-title">🇨🇳 国内互联网</div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Bilibili</div><div class="desc">B站主站</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Bilibili.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">腾讯</div><div class="desc">腾讯系服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Tencent.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">阿里巴巴</div><div class="desc">阿里系服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Alibaba.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">百度</div><div class="desc">百度系服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Baidu.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">网易</div><div class="desc">网易系服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/NetEase.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">字节跳动</div><div class="desc">字节跳动系（抖音等）</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/ByteDance.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">华为</div><div class="desc">华为系服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/HuaWei.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">小米</div><div class="desc">小米系服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Xiaomi.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">爱奇艺</div><div class="desc">爱奇艺视频</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Iqiyi.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">腾讯视频</div><div class="desc">腾讯视频</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/TencentVideo.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">优酷</div><div class="desc">优酷视频</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Youku.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">斗鱼</div><div class="desc">斗鱼直播</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Douyu.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">哔哩哔哩港澳台</div><div class="desc">Bilibili 港澳台限定</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/BilibiliHMT.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
            </div>
            <!-- 📦 开发者工具 -->
            <div class="rule-group">
              <div class="rule-group-title">📦 开发者工具</div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Docker</div><div class="desc">Docker 容器服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Docker.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">OneDrive</div><div class="desc">微软 OneDrive 云存储</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/OneDrive.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              ...
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Developer</div><div class="desc">开发者工具合集</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Developer.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
            </div>
            <!-- 💰 金融与加密 -->
            <div class="rule-group">
              <div class="rule-group-title">💰 金融与加密</div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Binance</div><div class="desc">币安交易所</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Binance.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">加密货币</div><div class="desc">加密货币相关服务合集</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Crypto.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
            </div>
            <!-- 📋 其他规则 -->
            <div class="rule-group">
              <div class="rule-group-title">📋 其他规则</div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">AI 合集</div><div class="desc">AI 服务综合规则合集</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/AI.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Adobe</div><div class="desc">Adobe 软件服务</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Adobe.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Amazon</div><div class="desc">亚马逊 AWS + 购物</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Amazon.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Wikipedia</div><div class="desc">维基百科</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Wikipedia.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
              <div class="rule-item"><label class="toggle"><input type="checkbox"><span class="slider"></span></label><div class="info"><div class="name">Pixiv</div><div class="desc">Pixiv 画师社区</div><div class="url">https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/Providers/Ruleset/Pixiv.yaml</div></div><span class="tag">ACL4SSR</span><span class="actions"><button class="edit" onclick="editRuleUrl(this)" title="编辑链接">✏️</button><button class="del" onclick="deleteRule(this)" title="删除规则">🗑</button></span></div>
            </div>
          </div>
        ...
        <button onclick="scrollTo('top')" title="回到顶部">⬆</button>
        <button onclick="scrollTo('bottom')" title="滚到底部">⬇</button>
    </div>
    </div>
    ...
    <div class="page" id="pageHelp" style="display:none">
      <div class="card">
        ...
        <div class="card-body" style="padding:20px;font-size:14px;line-height:1.8;color:var(--text2);">
          ...
          <p>2. 在 <strong>输出配置</strong> 中复制订阅地址，粘贴到 OpenClash 中</p>
          <p>3. 在 <strong>规则管理</strong> 中勾选你需要的规则集</p>
...
<script>
  // 这些函数已在下方 API 集成部分重新实现（使用真实后端）
  // 旧版 doLogin/logout 保留仅作为降级回退
  ...
  async function checkSession() {
    try {
      ...
      document.getElementById('userName').textContent = data.data?.username || 'admin';
  ...
  async function loadSubscriptions() {
    ...
    try {
      ...
      listEl.innerHTML = '<div style="padding:16px;text-align:center;color:var(--red);">加载失败</div>';
    }
  }
  ...
  window.loadOutputInfo = loadOutputInfo;
  ...
  async function loadOutputInfo() {
    try {
      ...
      document.getElementById('outputNodeCount').textContent = data.total + ' 个节点';
      renderOutputUrls(data.total);
    } catch (e) {
      document.getElementById('outputNodeCount').textContent = '加载失败';
    }
  }
  ...
  function renderOutputUrls(total) {
    ...
    const formats = [
      ...
      { key: 'clash', name: 'Clash', desc: 'Clash / OpenClash / Clash Verge', ext: 'yaml' },
      { key: 'base64', name: 'Base64', desc: '通用 Base64 节点列表', ext: 'txt' },
      { key: 'singbox', name: 'Sing-box', desc: 'Sing-box / Hiddify / NekoBox', ext: 'json' },
      { key: 'surge', name: 'Surge', desc: 'Surge iOS / Mac 配置', ext: 'conf' },
      { key: 'loon', name: 'Loon', desc: 'Loon iOS / Mac 配置', ext: 'conf' },
    ];

    const listEl = document.getElementById('outputUrlList');
    listEl.innerHTML = formats.map(f => {
      const url = token ? \`\${baseUrl}?format=\${f.key}&token=\${encodeURIComponent(token)}\` : \`\${baseUrl}?format=\${f.key}\`;
      return \`<div class="sub-item">
        <span class="status ok">🔗</span>
        <div style="flex:1;min-width:0;">
          <div style="display:flex;align-items:center;gap:8px;">
            <strong>\${f.name}</strong>
            <span style="font-size:11px;color:var(--text3);">\${f.desc}</span>
          </div>
          <div style="font-family:monospace;font-size:12px;color:var(--text2);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">\${url}</div>
        </div>
        <button class="copy-btn" onclick="copyToClipboard('\${url}')" style="padding:4px 10px;">复制</button>
      </div>\`;
    }).join('');
  }
  ...
  async function copyAllUrls() {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const token = urlParams.get('token') || '';
    const formats = ['auto', 'clash', 'base64', 'singbox', 'surge', 'loon'];
    const urls = formats.map(f => \`\${baseUrl}?format=\${f}\${token ? '&token=' + encodeURIComponent(token) : ''}\`);
  ...
  async function previewConfig() {
    ...
    try {
      const url = '/sub?format=' + format + (document.getElementById('subToken')?.value ? '&token=' + document.getElementById('subToken').value : '');
      const res = await fetch(url);
      const text = await res.text();
      pre.textContent = text;
    } catch (e) {
      pre.textContent = '预览失败: ' + e.message;
    }
  }
  ...
  async function loadOutputInfo() {
    try {
      ...
      document.getElementById('outputNodeCount').textContent = totalNodes + ' 个节点';
      renderOutputUrls(totalNodes > 0);
      
    } catch (e) {
      console.error('加载输出信息失败:', e);
      document.getElementById('outputNodeCount').textContent = '获取失败';
    }
  }

  function renderOutputUrls(hasNodes) {
    const listEl = document.getElementById('outputUrlList');
    const baseUrl = window.location.origin + '/sub';
    // 从 URL 获取 token 和 user
    const urlParams = new URLSearchParams(window.location.search);
    const token = urlParams.get('token') || '';
    const user = 'admin'; // 当前用户，简化处理
    
    const formats = [
      { key: 'auto', name: '🔄 自适应', desc: 'Clash YAML（自动适配）' },
      { key: 'clash', name: '🐱 Clash', desc: 'Clash / OpenClash / Clash Verge' },
      { key: 'base64', name: '📄 Base64', desc: '通用 Base64 节点列表' },
      { key: 'singbox', name: '📦 Sing-box', desc: 'Sing-box / sing-box-go' },
      { key: 'surge', name: '⚡ Surge', desc: 'Surge iOS / Mac' },
      { key: 'loon', name: '🚀 Loon', desc: 'Loon iOS / Mac' },
    ];

    listEl.innerHTML = formats.map(f => {
      const params = \`format=\${f.key}\${token ? '&token=' + encodeURIComponent(token) : ''}&user=\${user}\`;
      const url = \`\${baseUrl}?\${params}\`;
      return \`
        <div class="sub-item" style="flex-direction:column;align-items:flex-start;gap:6px;cursor:pointer;" onclick="showQrCode('\${url}', '\${f.name}')">
          <div style="display:flex;align-items:center;gap:10px;width:100%;">
            <strong style="min-width:100px;">\${f.name}</strong>
            <span style="font-size:12px;color:var(--text2);flex:1;">\${f.desc}</span>
            <input type="text" readonly value="\${url}" style="flex:1;font-size:12px;padding:6px 10px;background:var(--bg);" onclick="event.stopPropagation()">
            <button class="copy-btn" onclick="event.stopPropagation();copyToClipboard('\${url}', this)">复制</button>
          </div>
        </div>
      \`;
    }).join('');
  ...
  async function copyAllUrls() {
    ...
    const token = urlParams.get('token') || '';
    const formats = ['auto', 'clash', 'base64', 'singbox', 'surge', 'loon'];
    const urls = formats.map(f => \`\${baseUrl}/sub?format=\${f}\${token ? '&token=' + encodeURIComponent(token) : ''}\`);
    await copyToClipboard(urls.join('\\n'), document.querySelector('.output-actions .secondary'));
  }
  ...
  async function previewConfig() {
    ...
    try {
      const urlParams = new URLSearchParams(window.location.search);
      const token = urlParams.get('token') || '';
      const url = '/sub?format=' + format + (token ? '&token=' + encodeURIComponent(token) : '');
      const res = await fetch(url);
      const text = await res.text();
      pre.textContent = text;
    } catch (e) {
  ...
  document.addEventListener('click', function(e) {
    const title = e.target.closest('.rule-group-title');
    if (title) {
  ...
  // 编辑规则集 URL
  function editRuleUrl(btn) {
    const item = btn.closest('.rule-item');
    const urlEl = item.querySelector('.url');
    const nameEl = item.querySelector('.name');
    const currentUrl = urlEl.textContent.trim();
    const newUrl = prompt('编辑规则集引用链接：', currentUrl);
  ...
  window.editRuleUrl = editRuleUrl;
  ...
  function showQrCode(url, name) {
    ...
    qrImg.innerHTML = \`<img src="https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=\${encodeURIComponent(url)}" alt="QR Code" style="border-radius:4px;" onerror="this.parentElement.textContent='二维码加载失败'">\`;
  }
  ...
  async function copyQrCodeUrl() {
    try {
      ...
      const btn = document.querySelector('#qrcodeModal .primary');
  ...
  window.copyQrCodeUrl = copyQrCodeUrl;
  window.refreshNodeHealth = refreshNodeHealth;
</script>
</body>
</html>`;

      ...
      <div class="card">
        ...
        <div class="section" style="margin-top:20px;">
          ...
          <div class="output-actions">
            ...
            <button class="secondary" onclick="copyAllUrls()">📋 全部复制</button>
            <button class="secondary" onclick="loadOutputInfo()">🔄 刷新节点数</button>
          </div>
        </div>
...
<script>
  ...
  function renderOutputUrls(hasNodes) {
    ...
    listEl.innerHTML = formats.map(f => {
      const url = \`\${baseUrl}/sub?format=\${f.key}\${token ? '&token=' + token : ''}\`;
      return \`
        <div class="sub-item" style="flex-direction:column;align-items:flex-start;gap:6px;">
          <div style="display:flex;align-items:center;gap:10px;width:100%;">
            <strong style="min-width:100px;">\${f.name}</strong>
            <span style="font-size:12px;color:var(--text2);flex:1;">\${f.desc}</span>
            <input type="text" readonly value="\${url}" style="flex:1;font-size:12px;padding:6px 10px;background:var(--bg);">
            <button class="copy-btn" onclick="copyToClipboard('\${url}', this)">复制</button>
          </div>
        </div>
      \`;
    }).join('');

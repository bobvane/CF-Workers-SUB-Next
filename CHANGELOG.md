...
### 前端 fetch 超时修复 — 公司网络黑洞不再白屏（用户现场反馈）
- **`api()` 函数加 AbortController 超时 8s**：`/api/auth/session` 在公司网络被黑洞/挂起时，8s 后自动截断 → 走 catch → 显示登录页，而非永久白屏
- **`checkSession()` catch 增强**：超时场景显示 toast "连接服务器超时，已显示登录页（请检查网络）"，让用户感知网络异常而非误认网站坏了
- 模拟验证：happy-dom 端到端复现 8s 挂起场景，验证修复后白屏→登录页降级有效
- 测试基线：386 ✅（新增 2 个模拟测试）
